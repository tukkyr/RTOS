...
#define XMODEM_EOG 0x1a /* Ctrl-z */

#define XMODEM_BLOCK_SIZE 128
#define XMODEM_RECV_INTERVAL 1600000
...
static int xmode_read_block(unsigned char block_number, char *buf)
{
...
long xmode_recv(char *buf)
{
	...
	while (1) {
		...
		if (c == XMODEM_EOT) { /* 受信終了 */
			...
			receiving++;
			r = xmode_read_block(block_number, buf);
			if (r < 0) { /* 受信エラー */
				serial_send_byte(SERIAL_DEFAULT_DEVICE, XMODEM_NAK);
			} else { /* 正常受信 */
				block_number++;
